...
void PlayerPhysiñUpdate()
{
	...
	DebugDraw(playerX, playerY, PlayerSizeX, PlayerSizeY,0,0,1);
}
...
void Collision(int collisDir)// 1-test by x 0- test by y
{
	...
	{
		for (int iy = playerY; iy < (playerY + PlayerSizeY); iy++)
		{
	...
	for (int ix = playerX; ix < (playerX + PlayerSizeX); ix++)
	{
		for (int iy = playerY+0.5; iy < (int)(playerY + 0.5 + PlayerSizeY); iy++)
		{
			char tile = MapGetTile(ix, iy);
